...
def run_gpbo_loop(
    ...
    try:
        ls = None
        # Try ScaleKernel(base_kernel=...) path first, then direct covar_module
        for _get_ls in [
            lambda m: m.covar_module.base_kernel.lengthscale,
            lambda m: m.covar_module.lengthscale,
        ]:
            try:
                _ls = _get_ls(model)
                if _ls is not None:
                    arr = _ls.detach().cpu().numpy()
                    # Multi-output GP: shape (n_tasks, 1, n_dims) — take min
                    # across tasks (most sensitive output per dimension)
                    if arr.ndim == 3:
                        arr = arr.min(axis=0)
                    ls = arr.flatten()
                    break
