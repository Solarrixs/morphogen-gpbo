...
\lettrine{H}{}uman brain organoids---three-dimensional neural tissues derived from pluripotent stem cells---have emerged as powerful model systems for studying neurodevelopment, neurological disease, and drug response \cite{Lancaster2013, Pasca2019, Eze2021}. By modulating signaling pathways during early differentiation, researchers can steer organoids toward specific brain regions, generating cortical, thalamic, midbrain, and other region-specific neural tissues \cite{Amin2024}. However, the morphogen cocktails that control this patterning operate in a high-dimensional combinatorial space: a typical protocol involves more than 20 signaling molecules spanning WNT, BMP, SHH, FGF, retinoic acid, Notch, and EGF pathways, each with continuous concentration and timing parameters \cite{Amin2024, SanchisCalleja2025}.
...
\begin{enumerate}[nosep,leftmargin=*]
  ...
  \item \textbf{Multi-fidelity data integration.} We combine real experimental data (fidelity $f{=}1.0$) with computationally generated virtual data from CellRank~2 temporal projections ($f{=}0.5$) \cite{Lange2022cellrank, Weiler2024cellrank2, Klein2025moscot} and CellFlow generative modeling ($f{=}0.0$) \cite{Klein2025cellflow}, with automatic cross-fidelity correlation gating.
...
\textbf{AI disclosure.} Portions of the software implementation and manuscript drafting were assisted by large language models (Claude, Anthropic). All generated code was reviewed, tested (866 automated tests), and validated by the authors. All scientific conclusions, experimental design decisions, and data interpretations are the sole responsibility of the authors. No AI-generated content was used to fabricate or manipulate experimental data.
...
\subsection*{NEST-inspired transcriptomic maturity scoring}

To address the known limitation that cell type proportions alone do not capture transcriptomic maturity \cite{He2024hnoca, Naas2025nest}, we compute a per-condition NEST-inspired score using the mean KNN latent distance from the atlas mapping step. For each cell, the mean Euclidean distance to its $k{=}50$ nearest reference neighbors in the \scpoli{} latent space serves as a proxy for transcriptomic similarity: cells transcriptomically close to their fetal reference counterparts have low latent distances. We aggregate per condition as $s_\text{NEST} = \exp(-\bar{d}_c / \tilde{d})$, where $\bar{d}_c$ is the condition mean and $\tilde{d}$ is the global median. This score is included in the composite fidelity when available (weight 0.15, with other sub-scores re-normalized).
...
\subsection*{Experimental design and plate layout}

\textbf{Carry-forward controls.} Between optimization rounds, the top-$K$ conditions by observed fidelity are carried forward as positive controls (\texttt{--n-controls}, default $K{=}2$), following the robotic BO protocol of Kanda et al.\ \cite{Kanda2022}. These controls serve dual purposes: validating reproducibility across batches and providing batch-to-batch calibration anchors for the GP noise model.

\textbf{Failed experiment handling.} An optional \texttt{status} column in the training CSVs allows marking conditions as \texttt{valid}, \texttt{failed}, or \texttt{invalid}. Failed conditions are excluded from GP fitting but retained in the dataset for provenance tracking, following the floor-padding strategy of Morikawa et al.\ \cite{Morikawa2022}.

\textbf{LHD gap-filling.} For Round~2 onwards, a configurable number of plate slots (\texttt{--n-lhd-fill}) are filled with Latin Hypercube Design points using \texttt{scipy.stats.qmc.LatinHypercube}, ensuring space-filling coverage of under-explored morphogen regions alongside the BO-recommended candidates.

\textbf{Contextual BO with fixed harvest day.} Harvest day and base media concentrations can be declared as contextual parameters (\texttt{--contextual-cols}) that are held constant within a plate via \botorch{}'s \texttt{fixed\_features} mechanism. This reflects the physical constraint that all wells on a single plate share the same harvest day and base media composition.

\textbf{Cost-aware desirability gate.} An optional cost-aware desirability gate (\texttt{--cost-weight}) re-ranks BO recommendations using a Derringer--Suich geometric mean formulation \cite{DerringerSuich1980}: $D_i = a_i \times (1 - w_c \times \hat{c}_i)$, where $a_i$ is the acquisition value, $\hat{c}_i$ is the normalized cocktail cost, and $w_c$ is the user-specified cost weight. Morphogen costs per \si{\micro\Molar} are derived from vendor pricing, with recombinant proteins (${\sim}100{\times}$) more expensive than small molecules.

\textbf{Validation and confirmation plates.} The pipeline generates two specialized plate layouts for experimental validation. A \emph{validation plate} (\texttt{--validation}) tests the top 6 cocktails at half-log dose spacing ($0.3{\times}$, $1{\times}$, $3{\times}$) plus 6 replicate wells, probing dose-response sensitivity around predicted optima. A \emph{confirmation plate} (\texttt{--confirmation}) replicates the predicted optimum ($n{=}3$) alongside diverse runner-up candidates selected by max-min Euclidean distance, enabling statistical confirmation of the top recommendation.
...
\textbf{Confidence-based heteroscedastic noise.} CellFlow prediction confidence scores are converted to per-point observation noise variance via $\sigma^2_i = \sigma^2_\text{base} / c_i^2$, where $c_i \in (0, 1]$ is the confidence score computed from the distance to the nearest training points in morphogen space. This heteroscedastic noise model ensures the GP trusts high-confidence virtual predictions more than low-confidence ones, and is passed to the GP via the \texttt{train\_Yvar} parameter in \botorch{}.

\textbf{CellRank~2 transport quality.} Transport quality from CellRank~2 projections is monitored via the optimal transport cost and convergence status of each timepoint transition. Transitions flagged as \texttt{NOT\_CONVERGED} receive $100{\times}$ noise inflation, and \texttt{HIGH\_COST} transitions receive $10{\times}$, ensuring that unreliable temporal projections are down-weighted in the GP.

\textbf{Sanchis-Calleja multi-fidelity integration.} Data from the Sanchis-Calleja/Azbukina patterning screen \cite{SanchisCalleja2025} can be incorporated as an additional fidelity level ($f{=}0.7$, reflecting the cross-lab and cross-timepoint differences). Auto-discovery of the processed CSVs and fidelity-level configuration are controlled via \texttt{--sanchis-fractions}/\texttt{--sanchis-morphogens}/\texttt{--sanchis-fidelity} flags.

\textbf{Noise robustness characterization.} Empirical noise characteristics are computed per fidelity level via \texttt{characterize\_fidelity\_noise()}, which reports the mean and median column-wise variance and coefficient of variation across conditions at each fidelity. This diagnostic identifies whether low-fidelity data is too noisy to be informative before it enters the GP.
...
\textbf{d},~Brain region assignment across all conditions, showing 5 distinct brain regions plus unspecific/mixed.}
\label{fig:fidelity}
\end{figure*}
...
A5 & 2.422 & 0.0032 & 0.318 & 0.50 & Replicate \\
A6 & 2.489 & 0.0032 & 0.634 & 3.92 & Replicate \\
\bottomrule
\end{tabular}
\end{table}
