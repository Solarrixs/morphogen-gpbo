...
- [x] Phase B Idea #2: TVR (Targeted Variance Reduction) — Fit separate GPs per fidelity, scale variance by inverse cost, select model with lowest scaled variance at each candidate point. Add --tvr flag to 04_gpbo_loop.py. | Acceptance: --tvr produces recommendations; 5+ new tests
- [x] Phase B Idea #4: Target profile refinement — After Round 1, update target_profile using observed best compositions interpolated toward Braun reference. Wire into run_gpbo_loop() as --refine-target flag. | Acceptance: refined target differs from original; 3+ tests
- [x] Phase B Idea #12: FBaxis_rank regionalization — Extract A-P axis score from Sanchis-Calleja data. Add as continuous optimization target alongside discrete region profiles. | Acceptance: --target-region accepts "ap_axis" value; 3+ tests
...
- [x] Phase C Idea #10: Morphogen timing window encoding — Add temporal window categorical dimensions (early/mid/late patterning) to morphogen matrix. Use MixedSingleTaskGP for mixed continuous+categorical. | Acceptance: timing dims in training data; 3+ tests
- [x] Phase C Idea #11: Per-cell-type GP models — Fit separate GP per cell type (MAP path, GPerturb 2025). Per-output lengthscale matrix for interpretability. Compare to current multi-output approach. | Acceptance: per-type GPs produce predictions; 4+ tests
- [x] Phase D Idea #13: Per-round fidelity monitoring — Re-evaluate cross-fidelity correlation each round. Auto-fallback to single-fidelity if correlation degrades. Add trend to visualization report. | Acceptance: monitoring runs per round; 2+ tests
- [x] Phase D Idea #16: Convergence diagnostics — Track posterior variance, acquisition decay, recommendation clustering. Adaptive batch sizing. Add to gp_model_diagnostics.csv and viz report. | Acceptance: diagnostics in CSV; 4+ tests
